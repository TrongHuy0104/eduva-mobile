...
const EditQuestionModal: React.FC<EditQuestionModalProps> = ({ visible, onClose, questionId, initialTitle, initialContent, lessonMaterialId }) => {
  const {  mutate, isPending } = useUpdateQuestion()
    ...
    React.useEffect(() => {
      if (visible) {
        ...
        const processedContent = replacePImageWithImg(initialContent);
        setContent(processedContent);
        richText.current?.setContentHTML(processedContent);
        
      }
    }, [visible, initialTitle, initialContent]);

    const [title, setTitle] = useState(initialTitle);
    const [content, setContent] = useState(replacePImageWithImg(initialContent));

  // Helper function to replace <p-image ... /> with <img ... />
  function replacePImageWithImg(html: string): string {
    // Replace all <p-image ... /> or <p-image ...></p-image> with <img ... />
    // This regex matches both self-closing and paired tags, flexible to whitespace and attribute order
    ...
    const handleSubmit = () => {
      ...
      mutate({questionId, data: updateQuestionRequest}, {
        onSuccess: () => {
          setTitle("");
          setContent("");
          onClose();
