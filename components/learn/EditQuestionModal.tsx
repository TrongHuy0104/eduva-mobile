...
const EditQuestionModal: React.FC<EditQuestionModalProps> = ({
    ...
    const [content, setContent] = useState(
        replacePImageWithImg(initialContent)
    );
    ...
    const handleImageUpload = async () => {
        ...
        newImages.forEach((imageSrc) => {
            richText.current?.insertHTML(
                `<img src="${imageSrc}" style="max-width:100%;border-radius:8px;margin:8px 0;" />`
            );
        });
    };
    ...
    const handleSubmit = () => {
        ...
        mutate(
            { questionId, data: updateQuestionRequest },
            {
                onSuccess: () => {
                    setTitle('');
                    setContent('');
                        ...
                        <View style={styles.linkDialogActions}>
                            ...
                            <Pressable
                                onPress={() => {
                                    ...
                                    if (!url) {
                                        setLinkUrlError(
                                            'Vui lòng nhập URL liên kết!'
                                        );
                                        return;
                                    }
                                    // Add protocol if missing
                                    const urlPattern = /^https?:\/\//i;
                                    const normalizedUrl = urlPattern.exec(url)
                                        ? url
                                        : `https://${url}`;
