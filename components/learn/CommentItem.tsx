...
import { ActivityIndicator, Pressable, StyleProp, StyleSheet, Text, TouchableOpacity, useWindowDimensions, View, ViewStyle } from 'react-native';
...
export default function CommentItem({
    ...
    const handleReplySubmit = async () => {
      console.log(111);
        ...
        try {
          const createCommentRequest: CreateCommentRequest = {
            ...
            content,
            parentCommentId: data.parentCommentId ? data.parentCommentId : commentId!,
          };
      ...
      const handleEditSubmit =  () => {
        ...
        try {
          ...
          updateCommentMutation({commentId: data.id, data: updateCommentRequest}, {
            onSuccess: () => {
              setIsEditing(false);
            },
          });
    ...
    return (
        <View style={[styles.commentContainer, {backgroundColor: isBestComment ? '#48bd790a' : undefined, borderWidth: isBestComment ? 1 : undefined, borderColor: isBestComment ? '#48bd7929' : undefined}, style]}>
            {/* Question owner */}
            <View style={styles.questionOwner}>
                <View style={styles.questionOwnerInfo}>
                    ...
                    />
                    <View>
                        <Text style={[styles.questionOwnerName, {maxWidth: width * 0.45}]} numberOfLines={1} ellipsizeMode="tail">{data.createdByName}</Text>
                        <Text style={styles.questionOwnerTime}>{formatDateVi(data.createdAt)}</Text>
                    </View>
                </View>
            
                <View style={{flexDirection: 'row', alignItems: 'center', gap: 4, position: 'relative', top: 4}}>
                    {isBestComment && (
                        <Image
                            style={{
                                width: 16,
                    ...
                    {isBestComment && (
                      <Text style={{color: '#48bd79', maxWidth: width * 0.25}}>
                        Bình luận
                        {'\n'}
                    ...
                    <View style={styles.replyActions}>
                        ...
                        <TouchableOpacity
                            style={[styles.button,
                                    ...
                                    (!commentEditContent?.trim() || isUpdatingComment) && styles.disabledButton
                                  ]}
                            onPress={handleEditSubmit}
                            disabled={!commentEditContent?.trim() || isUpdatingComment}
                        >
                          {isUpdatingComment && (
            ...
            <View style={{flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 12, justifyContent: 'space-between'}}>
                {/* Reply */}
                <Pressable 
                  style={({pressed}: {pressed: boolean}) => [{
                    opacity: pressed ? 0.8 : 1,
                  }, {flexDirection: 'row', alignItems: 'center', gap: 4, position: 'relative', top: -2}]} 
                                ...
                                <View style={{ height: 1, backgroundColor: 'rgba(255, 255, 255, 0.1)' }} />
                                {
                                  data.canDelete && (
                                    ...
                                    >
                                        ...
                                        <Text style={{ color: '#ff4d4f', fontSize: 14 }}>Xóa</Text>
                                    </Pressable>
                                  )
                                }
                            </View>
            ...
            {/* Reply Input */}
            {showReplyInput && (
                <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 8, marginTop: 8, marginLeft: 16 }}>
                  <Image 
                  ...
                  <View style={styles.replyInputContainer}>
                    <RichTextEditor
                        ...
                        style={styles.replyEditor}
                        initialContent={user?.id === data.createdByUserId 
                          ? '' 
                    ...
                    <View style={styles.replyActions}>
                        ...
                        <TouchableOpacity
                            style={[styles.button,
                                    styles.primaryButton,
                                    (!commentCreateContent?.trim() || isCreatingComment) && styles.disabledButton
                                  ]}
                            onPress={handleReplySubmit}
                            disabled={!commentCreateContent?.trim() || isCreatingComment}
                        >
                          {isCreatingComment && (
                            <ActivityIndicator color="#fff" style={{ marginRight: 8 }} />
                          )}
